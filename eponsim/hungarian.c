...
/*
   $Log: hungarian.c,v $
   Revision 1.1  2007/03/30 21:03:45  mmcgarry
   Hungarian solver for assignment problem from Dr. Stachniss and Stanford GraphBase.
...
/*
 *
 * Function: hungarian_assignment()
 * Purpose:  Given a row number, return the assigned column
 * Author:   Michael P. McGarry
 *
 */
int hungarian_assignment(hungarian_problem_t* p, int row)
{
	int col;
	/* Look for the assigned column for this row */
...
void hungarian_solve(hungarian_problem_t* p)
{
  ...
  if((col_mate == NULL) || (unchosen_row == NULL) || (row_dec == NULL) || (slack_row == NULL))
  {
	  printf("Memory allocation failed. num_rows = %d\n",p->num_rows);
  }
  ...
  if((row_mate == NULL) || (parent_row == NULL) || (col_inc == NULL) || (slack == NULL))
  {
	  printf("Memory allocation failed. num_cols = %d\n",p->num_cols);
  }
  ...
  for (k=0;k<m;k++)
    {
      ...
      {
          printf("Second case causes exit\n");
          fflush(NULL);
          exit(0);
      }
    }
